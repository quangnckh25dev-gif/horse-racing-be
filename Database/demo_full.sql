/* ============================================================
   DEMO_FULL.SQL — 1 FILE DUY NHAT: reset + tao DB demo hoan chinh
   CHAY (BAT BUOC -f 65001 de dung UTF-8 tieng Viet):
     sqlcmd -S localhost,1433 -U sa -P 12345 -C -f 65001 -i demo_full.sql
   Gom: schema + seed goc  +  lam dep (ten VN, vi=0, jockey stats, 2 tk pending)
        + referee3/4 + jockey5/6 (co ti le thang)
        + 1 giai 3 vong Qualify(8 ngua)->Semi Final(6)->Final de demo loai ngua
   ============================================================ */
-- ============================================================
...
DECLARE @pw NVARCHAR(200) = '$2a$10$6rvu1cSRS60NNTQtJQZpYO34ZCaJ73I8dFDvXdw4BxYzrlKFKhTq6';
DECLARE @refRole INT = (SELECT RoleID FROM Roles WHERE RoleName='Referee');
DECLARE @jkRole  INT = (SELECT RoleID FROM Roles WHERE RoleName='Jockey');
DECLARE @uid INT;
DECLARE @orgId INT = (SELECT UserID FROM Users WHERE Username='organizer1');
DECLARE @tid INT, @rQ INT, @rS INT, @rF INT, @raceQ INT, @raceS INT;

/* ===== Referee bo sung (referee3, referee4) ===== */
IF NOT EXISTS (SELECT 1 FROM Users WHERE Username='referee3') BEGIN
  INSERT INTO Users(Username,PasswordHash,FullName,Email,RoleID,IsActive,IsApproved) VALUES('referee3',@pw,N'Trọng Tài C','referee3@demo.com',@refRole,1,1);
  SET @uid=SCOPE_IDENTITY();
  INSERT INTO Referees(UserID,BadgeNumber,Speciality) VALUES(@uid,'RF-003',N'Đường đua bằng');
END
IF NOT EXISTS (SELECT 1 FROM Users WHERE Username='referee4') BEGIN
  INSERT INTO Users(Username,PasswordHash,FullName,Email,RoleID,IsActive,IsApproved) VALUES('referee4',@pw,N'Trọng Tài D','referee4@demo.com',@refRole,1,1);
  SET @uid=SCOPE_IDENTITY();
  INSERT INTO Referees(UserID,BadgeNumber,Speciality) VALUES(@uid,'RF-004',N'Đường đua bằng');
END

/* ===== Jockey bo sung (co ti le thang) ===== */
IF NOT EXISTS (SELECT 1 FROM Users WHERE Username='jockey5') BEGIN
  INSERT INTO Users(Username,PasswordHash,FullName,Email,RoleID,IsActive,IsApproved) VALUES('jockey5',@pw,N'Nài Ngựa E','jockey5@demo.com',@jkRole,1,1);
  SET @uid=SCOPE_IDENTITY();
  INSERT INTO Jockeys(UserID,NationalID,LicenseNumber,WeightKg,HeightCm,ExperienceYear,ApprovalStatus,TotalRaces,TotalWins) VALUES(@uid,'079205000105','JK-005',52.0,165,5,'Approved',20,11);
END
IF NOT EXISTS (SELECT 1 FROM Users WHERE Username='jockey6') BEGIN
  INSERT INTO Users(Username,PasswordHash,FullName,Email,RoleID,IsActive,IsApproved) VALUES('jockey6',@pw,N'Nài Ngựa F','jockey6@demo.com',@jkRole,1,1);
  SET @uid=SCOPE_IDENTITY();
  INSERT INTO Jockeys(UserID,NationalID,LicenseNumber,WeightKg,HeightCm,ExperienceYear,ApprovalStatus,TotalRaces,TotalWins) VALUES(@uid,'079205000106','JK-006',50.5,162,3,'Approved',8,4);
END

/* ===== Giai demo 3 vong: Qualify (8 ngua) -> Semi Final (6 ngua) -> Final ===== */
IF NOT EXISTS (SELECT 1 FROM Tournaments WHERE TournamentName=N'Giải Vô Địch Quốc Gia 2026')
BEGIN
  INSERT INTO Tournaments(TournamentName,Description,Location,StartDate,EndDate,BudgetTotal,MaxHorses,MaxParticipants,Status,CreatedBy)
    VALUES(N'Giải Vô Địch Quốc Gia 2026',N'Demo vòng loại/bán kết/chung kết',N'Phú Thọ Racecourse (HCMC)','2026-08-01','2026-08-20',500000000,8,8,'Open',@orgId);
  SET @tid=SCOPE_IDENTITY();

  INSERT INTO Rounds(TournamentID,RoundName,RoundOrder,StartDate,EndDate) VALUES(@tid,N'Qualify',1,'2026-08-01','2026-08-05');    SET @rQ=SCOPE_IDENTITY();
  INSERT INTO Rounds(TournamentID,RoundName,RoundOrder,StartDate,EndDate) VALUES(@tid,N'Semi Final',2,'2026-08-08','2026-08-12'); SET @rS=SCOPE_IDENTITY();
  INSERT INTO Rounds(TournamentID,RoundName,RoundOrder,StartDate,EndDate) VALUES(@tid,N'Final',3,'2026-08-15','2026-08-20');      SET @rF=SCOPE_IDENTITY();

  INSERT INTO Races(TournamentID,RoundID,RaceName,RaceDate,TrackLength,TrackType,MaxParticipants,PrizeFirst,PrizeSecond,PrizeThird,Status,RegistrationOpen,RegistrationClose)
    VALUES(@tid,@rQ,N'Vòng Loại','2026-08-05T08:00:00',1600,'Flat',8,30000000,15000000,7000000,'Finished','2026-08-01T08:00:00','2026-08-04T08:00:00'); SET @raceQ=SCOPE_IDENTITY();
  INSERT INTO Races(TournamentID,RoundID,RaceName,RaceDate,TrackLength,TrackType,MaxParticipants,PrizeFirst,PrizeSecond,PrizeThird,Status,RegistrationOpen,RegistrationClose)
    VALUES(@tid,@rS,N'Bán Kết','2026-08-12T08:00:00',1600,'Flat',6,30000000,15000000,7000000,'Draft','2026-08-08T08:00:00','2026-08-11T08:00:00'); SET @raceS=SCOPE_IDENTITY();

  -- 8 entry vong Qualify (6 con co jockey Ready, 2 con Approved chua co jockey)
  INSERT INTO RaceEntries(RaceID,HorseID,JockeyID,LaneNumber,RegistrationStatus,OrganizerApproved,ApprovedBy,JockeyConfirmed,Odds) VALUES
   (@raceQ,1,1,1,'Ready',1,@orgId,1,2.5),
   (@raceQ,2,2,2,'Ready',1,@orgId,1,3.0),
   (@raceQ,3,3,3,'Ready',1,@orgId,1,3.5),
   (@raceQ,4,4,4,'Ready',1,@orgId,1,4.0),
   (@raceQ,5,5,5,'Ready',1,@orgId,1,4.5),
   (@raceQ,6,6,6,'Ready',1,@orgId,1,5.0),
   (@raceQ,7,NULL,7,'Approved',1,@orgId,0,6.0),
   (@raceQ,8,NULL,8,'Approved',1,@orgId,0,7.0);

  -- Ket qua (FinishPosition = LaneNumber), da Published
  INSERT INTO RaceResults(RaceID,EntryID,FinishPosition,Points,ApprovalStatus,PublishedAt,ConfirmedAt)
    SELECT @raceQ, EntryID, LaneNumber, (9-LaneNumber)*10, 'Published', GETDATE(), GETDATE()
    FROM RaceEntries WHERE RaceID=@raceQ;

  -- Xet vong: giu 3/4 (top 6) Qualified, loai 1/4 (2 con cuoi) Eliminated
  UPDATE RaceEntries SET RoundStatus='Qualified' WHERE RaceID=@raceQ AND LaneNumber<=6;
  UPDATE RaceEntries SET RoundStatus='Eliminated', EliminationRoundID=@rQ, EliminationReason=N'Qualify cutoff' WHERE RaceID=@raceQ AND LaneNumber>6;

  -- Tao 6 entry vong Semi Final tu ngua qualified
  INSERT INTO RaceEntries(RaceID,HorseID,JockeyID,LaneNumber,RegistrationStatus,OrganizerApproved,ApprovedBy,JockeyConfirmed,Odds,RoundStatus)
    SELECT @raceS, HorseID, JockeyID, ROW_NUMBER() OVER(ORDER BY LaneNumber), RegistrationStatus, 1, @orgId, JockeyConfirmed, Odds, 'Qualified'
    FROM RaceEntries WHERE RaceID=@raceQ AND RoundStatus='Qualified';
END

SELECT 'EXTRAS DONE' AS Result;
